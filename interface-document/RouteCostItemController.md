# RouteCostItemController 接口说明

## 1. 通用约定

- 对应表：`route_cost_items`；基础路径：`/api/route-cost-items`。
- 所有接口需要请求头：`Authorization: Bearer <token>`。
- 新增、更新使用 JSON 请求体，字段名为驼峰，`Content-Type: application/json`。
- 使用本模块独立权限 `route-cost-item:list`、`route-cost-item:create`、`route-cost-item:update`、`route-cost-item:delete` 权限。通过现有 RBAC 分配给调用用户；列表和详情共用查询权限。角色 ID 为 1 已关联本模块全部操作权限。
...
| --- | --- | --- | --- | --- |
| GET | `/api/route-cost-items` | 分页列表 | `route-cost-item:list` | 分页对象，包含 `records/total/pageNum/pageSize` |
| GET | `/api/route-cost-items/{id}` | 详情 | `route-cost-item:list` | 完整对象 |
| POST | `/api/route-cost-items` | 新增 | `route-cost-item:create` | 保存后的完整对象，包含自增 id |
| PUT | `/api/route-cost-items/{id}` | 全量更新 | `route-cost-item:update` | `null` |
| DELETE | `/api/route-cost-items/{id}` | 删除 | `route-cost-item:delete` | `null` |

列表支持可选参数 `routeId`，例如 `/api/route-cost-items?routeId=route-001`。不传时不限制所属路线，按自增 ID 升序分页。指定路线不存在时返回 `code=404`，空白路线 ID 返回 `code=400`。
...
| `costType` | string | 是 | 路线费用类型；ticket / shuttle / waste / parking / other |
...
```json
{
  ...
  "minCny": 10,
  "maxCny": 30,
  "sourceUrl": "https://example.com/cost"
}
```

更新时向 `PUT /api/route-cost-items/1` 提交上述业务字段，修改需要调整的值。
